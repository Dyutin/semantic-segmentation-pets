...
def apply_brightness_increase(img_tensor, value):
...
def evaluate_on_brightness_increase(data_path, model_path, device):
    ...
    for val in brightness_levels:
        ...
        with torch.no_grad():
            for img, gt_mask in test_loader:
                ...
                perturbed_imgs = torch.stack([apply_brightness_increase(im, val) for im in img])
    ...
    plt.xlabel("Brightness Increase (Pixel Value Added)")
    plt.ylabel("Mean Dice Score")
    plt.title("Segmentation Performance vs Brightness Increase")
    plt.grid(True)
    plt.savefig("mean_dice_vs_brightness_increase.png", bbox_inches="tight")
    plt.close()
...
if __name__ == "__main__":
    ...
    evaluate_on_brightness_increase(DATA_PATH, MODEL_PATH, device)

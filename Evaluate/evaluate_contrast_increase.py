...
def apply_contrast_increase(img_tensor, factor):
...
def evaluate_on_contrast_increase(data_path, model_path, device):
    contrast_levels = [1.0, 1.01, 1.02, 1.03, 1.04, 1.05, 1.1, 1.15, 1.20, 1.25]
    ...
    for factor in contrast_levels:
        ...
        with torch.no_grad():
            for img, gt_mask in test_loader:
                ...
                perturbed_imgs = torch.stack([apply_contrast_increase(im, factor) for im in img])
    ...
    plt.title("Segmentation Performance vs Contrast Increase")
    plt.grid(True)
    plt.savefig("mean_dice_vs_contrast_increase.png", bbox_inches="tight")
    plt.close()
...
if __name__ == "__main__":
    ...
    evaluate_on_contrast_increase(DATA_PATH, MODEL_PATH, device)

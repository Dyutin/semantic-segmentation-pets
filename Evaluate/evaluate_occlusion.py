...
def apply_occlusion(img_tensor, occ_size):
    ...
    _, H, W = img.shape
    if occ_size > 0:
        top = random.randint(0, max(0, H - occ_size))
        left = random.randint(0, max(0, W - occ_size))
        img[:, top:top+occ_size, left:left+occ_size] = 0.0

    img = (img - mean) / std
    return img.to(img_tensor.device)

def evaluate_on_occlusion(data_path, model_path, device):
    ...
    for occ_size in occlusion_sizes:
        ...
        with torch.no_grad():
            for img, gt_mask in test_loader:
                ...
                perturbed_imgs = torch.stack([apply_occlusion(im, occ_size) for im in img])
        ...
        print(f"Occlusion {occ_size}x{occ_size}: Mean Dice = {mean_dice:.4f}")

    # Save Plot
    plt.figure(figsize=(8, 6))
    plt.plot(occlusion_sizes, mean_dice_scores, marker='o')
    plt.xlabel("Occlusion Square Size")
    plt.ylabel("Mean Dice Score")
    plt.title("Segmentation Performance vs Image Occlusion")
    plt.grid(True)
    plt.savefig("mean_dice_vs_occlusion.png", bbox_inches="tight")
    plt.close()
...
if __name__ == "__main__":
    ...
    evaluate_on_occlusion(DATA_PATH, MODEL_PATH, device)
